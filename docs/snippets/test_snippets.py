...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('docs/snippets/miss_event/miss_event-001.py', b'OK', ['redis']),
        ('docs/snippets/miss_event/miss_event-002.py', b'OK', ['redis']),
    ],
)

def test_snippet(standalone, snippet, expected, requirements):
    with open(snippet, 'rb') as f:
        src = f.read()
    standalone.flushall()
    if len(requirements) > 0:
        requirements = ['REQUIREMENTS'] + requirements
    r = standalone.execute_command('RG.PYEXECUTE', src, *requirements)
