...
* _id_: the [execution ID](functions.md#executionid) to abort
...
* _id_: the [execution ID](functions.md#executionid) to remove
...
* **executionId**: the [execution ID](functions.md#executionid)
* **status**: the status (e.g. 'pending', 'running', 'done'?)
...
* **id**: the [registration ID](functions.md#registrationid)
...
* _id_: the [execution ID](functions.md#executionid) to get
...
* **execution_plan**: the plan of execution is made of:
    ...
    * **steps**: the plan's steps are made of the following:
        * **type**: step type
        * **duration**: the step's duration in milliseconds
        * **name**: step callback
        * **arg**: step argument
...
The **RG.GETRESULTSBLOCKING** command cancels the `UNBLOCKING` subcommand of the [`RG.PYEXECUTE`](#rgpyexecute) command. The calling client is blocked until execution ends and is sent with any results and errors then.
...
* _id_: the [execution ID](functions.md#executionid) to block upon
...
* _REQUIREMENTS_: this subcommand ensures that list of dependencies it is given as an argument is installed on each shard before execution
...
When used in `UNBLOCKING` mode reply is an [execution ID](functions.md#executionid).
...
* **id**: the [registration ID](functions.md#registrationid) for removal
