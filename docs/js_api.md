...
The following is a `.d.ts` files that describe the API given by RedisGears. Add it to your project along side the JS file bellow to get a full auto complete capability on your RedisGears project. Example (assuming you call the files `gears-api.d.ts` and `gears-api.js`):

```JS
#!js name=lib api_version=1.1

import {redis} from './gears-api';
