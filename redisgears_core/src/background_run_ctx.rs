...
use crate::{verify_ok_on_replica, verify_oom, get_libraries, GearsLibraryMataData};
...
impl RemoteTask for GearsRemoteTask {
    ...
    fn task(
        ...
        let library = {
            ...
            if library.is_none() {
                on_done(Err(format!("Library {} does not exists on remote shard", self.lib_name)));
                return;
            }
        ...
        if remote_function.is_none() {
            on_done(Err(format!("Remote function {} does not exists on library {}", self.job_name, self.lib_name)));
            return;
        }
        let remote_function = remote_function.unwrap();
        remote_function(r.buff, Box::new(move |result|{
...
impl BackgroundRunFunctionCtxInterface for BackgroundRunCtx {
    ...
    fn run_on_key(&self, key: &[u8], job_name: &str, input: &[u8], on_done: Box<dyn FnOnce(Result<Vec<u8>, GearsApiError>)>) {
        ...
        let input_record = GearsBufferRecord{buff: input.into_iter().map(|v| *v).collect()};
        mr::libmr::remote_task::run_on_key(key, task, input_record, move |result: Result<GearsBufferRecord, RustMRError>|{
            let res = match result {
                Ok(r) => Ok(r.buff),
