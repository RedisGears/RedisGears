...
use stream_reader::{ConsumerData, StreamReaderCtx};

use compiled_library_api::CompiledLibraryInternals;
use keys_notifications::{KeysNotificationsCtx, NotificationCallback, NotificationConsumer};
use stream_run_ctx::{GearsStreamConsumer, GearsStreamRecord};

use std::cell::RefCell;

use keys_notifications::ConsumerKey;
...
pub const PSEUDO_SLAVE_READONLY_CONFIG_NAME: &str = "pseudo-slave-readonly";
pub const PSEUDO_SLAVE_CONFIG_NAME: &str = "pseudo-slave";

pub fn get_and_verify_redis_version_compatible(ctx: &Context) -> Result<RedisVersion, String> {
...
/// state information.
pub struct GearsLibraryCtx {
    pub(crate) meta_data: Arc<GearsLibraryMetaData>,
    pub(crate) functions: HashMap<String, GearsFunctionCtx>,
    pub(crate) remote_functions: HashMap<String, RemoteFunctionCtx>,
    pub(crate) stream_consumers:
        HashMap<String, Arc<RefCellWrapper<ConsumerData<GearsStreamRecord, GearsStreamConsumer>>>>,
    pub(crate) revert_stream_consumers:
        Vec<(String, GearsStreamConsumer, usize, bool, Option<String>)>,
    pub(crate) notifications_consumers: HashMap<String, Arc<RefCell<NotificationConsumer>>>,
    pub(crate) revert_notifications_consumers:
        Vec<(String, ConsumerKey, NotificationCallback, Option<String>)>,
    pub(crate) old_lib: Option<Arc<GearsLibrary>>,
}
...
impl GearsLibraryCtx {
    ...
    pub fn new(
        ...
        old_library: Option<Arc<GearsLibrary>>,
    ) -> Self {
        Self {
            meta_data,
...
struct GearsLoadLibraryCtx<'ctx, 'lib_ctx> {
    ...
    pub(crate) gears_lib_ctx: &'lib_ctx mut GearsLibraryCtx,
}

pub struct GearsLibrary {
    pub(crate) gears_lib_ctx: GearsLibraryCtx,
    pub(crate) lib_ctx: Box<dyn LibraryCtxInterface>,
    pub(crate) compile_lib_internals: Arc<CompiledLibraryInternals>,
}
...
pub type FutureHandlerContextCallback = dyn FnOnce(&Context, CallResult<'static>);
pub type FutureHandlerContextDisposer = dyn FnOnce(&Context, bool);
...
pub struct FutureHandlerContext {
...
pub(crate) fn get_globals() -> &'static GlobalCtx {
    unsafe { GLOBALS.as_ref().unwrap() }
}

pub(crate) fn get_globals_mut() -> &'static mut GlobalCtx {
