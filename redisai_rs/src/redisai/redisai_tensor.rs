use crate::redisai_raw::bindings::{
    RAI_Tensor,
    RedisAI_TensorCreate,
    RedisAI_TensorFree,
    RedisAI_TensorLength,
    RedisAI_TensorDataSize,
    RedisAI_TensorSetData,
    RedisAI_TensorData,
    RedisAI_TensorByteSize,
    RedisAI_TensorNumDims,
    RedisAI_TensorDim,
};
use std::ffi::CString;
...
impl RedisAITensor {
    pub fn create(data_type: &str, dims: &[i64]) -> Result<RedisAITensor, RedisAIError> {
        ...
        let inner_tensor = unsafe{RedisAI_TensorCreate.unwrap()(data_type_c_str.as_ptr(), dims.as_ptr() as *mut i64, dims.len() as i32)};
    ...
    pub fn set_data(&mut self, data: &[u8]) -> Result<(), RedisAIError> {
        if data.len() != self.bytes_len() {
            return Err(format!("Buffer size {} but expected {}", data.len(), self.bytes_len()));
        }
        if unsafe{RedisAI_TensorSetData.unwrap()(self.inner_tensor, data.as_ptr() as *const c_char, data.len())} != 1 as c_int {
