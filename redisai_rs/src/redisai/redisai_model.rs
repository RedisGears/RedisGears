use crate::redisai_raw::bindings::{
    ...
    RedisAI_GetError,
    RAI_ModelRunCtx,
    RedisAI_ModelRunCtxCreate,
    RedisAI_ModelRunCtxFree,
    RedisAI_ModelRunCtxAddInput,
    RedisAI_ModelRunAsync,
    RAI_OnFinishCtx,
    RedisAI_GetAsModelRunCtx,
    RedisAI_ErrorCode_RedisAI_ErrorCode_OK,
    RedisAI_GetErrorCode,
    RedisAI_ModelRunCtxNumOutputs,
    RedisAI_ModelRunCtxOutputTensor,
    RedisAI_TensorGetShallowCopy,
    RedisAI_ModelGetShallowCopy,
    RedisAI_ModelRunCtxAddOutput,
};
...
impl RedisAIModel{
    pub fn open_from_key(ctx: &Context, key_name: &str) -> Result<RedisAIModel, RedisAIError> {
        ...
        let res = unsafe{RedisAI_GetModelFromKeyspace.unwrap()(ctx.ctx as *mut RedisModuleCtx,
...
extern "C" fn model_run_done<Callback: FnOnce(Result<Vec<RedisAITensor>, RedisAIError>)>(ctx: *mut RAI_OnFinishCtx, private_data: *mut ::std::os::raw::c_void) {
    ...
    for i in 0 .. num_outputs {
        let inner_tensor = unsafe{RedisAI_TensorGetShallowCopy.unwrap()(RedisAI_ModelRunCtxOutputTensor.unwrap()(inner_run_ctx, i))};
...
impl RedisAIModelRunCtx {
    pub fn add_input(&mut self, name: &str, tensor: &RedisAITensor) -> Result<(), RedisAIError> {
        ...
        unsafe{RedisAI_ModelRunCtxAddInput.unwrap()(self.inner_run_ctx, name_c_string.as_ptr(), tensor.inner_tensor)};
        Ok(())
    }
    ...
    pub fn add_output(&mut self, name: &str) -> Result<(), RedisAIError> {
        ...
        let name_c_string = CString::new(name).unwrap();
        unsafe{RedisAI_ModelRunCtxAddOutput.unwrap()(self.inner_run_ctx, name_c_string.as_ptr())};
        Ok(())
    }
    ...
    pub fn run<Callback: FnOnce(Result<Vec<RedisAITensor>, RedisAIError>)>(&mut self, on_done: Callback) {
        if self.inner_run_ctx.is_null() {
            ...
            unsafe{RedisAI_ModelRunAsync.unwrap()(self.inner_run_ctx, Some(model_run_done::<Callback>), on_done as *mut c_void)};
...
impl AIModelRunnerInterface for RedisAIModelRunCtx {
    fn add_input(&mut self, name: &str, tensor: &dyn AITensorInterface) -> Result<(), GearsApiError> {
        ...
        self.add_input(name, tensor).map_err(|e| GearsApiError::Msg(e))
    }
