...
redis.registerAsyncFunction("test", async (async_client, key) => {
    ...
    for conn in shardsConnections(env):
        res = conn.execute_command('TFCALLASYNC', 'test', '1', 'x')
...
["raw-arguments"]);
    ...
    for conn in shardsConnections(env):
        try:
            conn.execute_command('TFCALLASYNC', 'test', '1', 'x')
            pass
...
redis.registerAsyncFunction("test", async(async_client, key) => {
    ...
    for conn in shardsConnections(env):
        res = conn.execute_command('TFCALLASYNC', 'test', '1', 'key0')
...
redis.registerAsyncFunction("test", async (async_client, key) => {
    ...
    """
    cluster_conn.execute_command('set', 'x', '1')
    env.expect('TFCALLASYNC', 'test', '1', 'x').error().contains('Remote task timeout')
...
redis.registerAsyncFunction("test", async(async_client) => {
    ...
    for conn in shardsConnections(env):
        res = conn.execute_command('TFCALLASYNC', 'test', '0')
...
redis.registerAsyncFunction("test", async (async_client) => {
    ...
    env.expect('TFCALLASYNC', 'test', '1', 'z').error().contains('Timeout')
