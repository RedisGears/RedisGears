...
redis.registerFunction("get", function(client, dummy, key){
    ...
    env.expect('TFCALL', 'get', '1', 'x', 'x').equal('1')
    env.expect('TFCALL', 'get', '1', 'x', 'cached:x').equal('1')
    env.expect('AUTH', 'alice', 'pass').equal(True)
    env.expect('TFCALL', 'get', '1', 'cached:x', 'x').error().contains(NO_PERMISSIONS_ERROR_MSG)
    env.expect('TFCALL', 'get', '1', 'cached:x', 'cached:x').equal('1')
...
redis.registerAsyncFunction("get", async function(client, dummy, key){
    ...
    env.expect('set', 'x', '1').equal(True)
    env.expect('set', 'cached:x', '1').equal(True)
    env.expect('TFCALLASYNC', 'get', '1', 'x', 'x').equal('1')
    env.expect('TFCALLASYNC', 'get', '1', 'x', 'cached:x').equal('1')
    env.expect('AUTH', 'alice', 'pass').equal(True)
    env.expect('TFCALLASYNC', 'get', '1', 'cached:x', 'x').error().contains(NO_PERMISSIONS_ERROR_MSG)
    env.expect('TFCALLASYNC', 'get', '1', 'cached:x', 'cached:x').equal('1')
...
redis.registerFunction("async_get_start", function(client, dummy, key){
    ...
    env.expect('set', 'x', '1').equal(True)
    env.expect('set', 'cached:x', '1').equal(True)
    env.expect('TFCALLASYNC', 'async_get_start', '1', 'cached:x', 'x').equal('OK')
    env.expect('TFCALLASYNC', 'async_get_continue', '0').equal('1')
    env.expect('TFCALLASYNC', 'async_get_start', '1', 'cached:x', 'cached:x').equal('OK')
    env.expect('TFCALLASYNC', 'async_get_continue', '0').equal('1')
    c = env.getConnection()
    c.execute_command('AUTH', 'alice', 'pass')

    env.assertEqual(c.execute_command('TFCALLASYNC', 'async_get_start', '1', 'cached:x', 'x'), "OK")
    try:
        c.execute_command('TFCALLASYNC', 'async_get_continue', '0')
        env.assertTrue(False, message='Command succeed though should failed')
    except Exception as e:
        env.assertContains(NO_PERMISSIONS_ERROR_MSG, str(e))

    env.assertEqual(c.execute_command('TFCALLASYNC', 'async_get_start', '1', 'cached:x', 'cached:x'), "OK")
    try:
        env.assertEqual(c.execute_command('TFCALLASYNC', 'async_get_continue', '0'), '1')
    except Exception as e:
